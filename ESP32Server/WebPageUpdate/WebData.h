/*
  ...
  now you can have the class define the look AND the class update the content
  but you will then need a class for every data field that must be updated, here's what that will look like:-
  ...
  <script type = "text/javascript">
    ...
    function response(){
      ...
      if (message == 0){
        ...
        document.getElementById("switch0").innerHTML="Switch is OFF";
      }
      else{
        document.getElementById("btn0").innerHTML="Turn OFF";
        document.getElementById("switch0").innerHTML="Switch is ON";
      }

      //SWITCH
      xmldoc = xmlResponse.getElementsByTagName("DEVICE1");
      message = xmldoc[0].firstChild.nodeValue;
      document.getElementById("switch1").style.backgroundColor="rgb(200,200,200)";
      // update the text in the table
      if (message == 0){
        document.getElementById("switch1").innerHTML="Switch is OFF";
        document.getElementById("btn1").innerHTML="Turn ON";
        document.getElementById("switch1").style.color="#0000AA"; 
      }
      else {
        document.getElementById("switch1").innerHTML="Switch is ON";
        document.getElementById("btn1").innerHTML="Turn OFF";
        document.getElementById("switch1").style.color="#00AA00";
      }
    }
  
    // general processing code for the web page to ask for an XML steam
    // this is actually why you need to keep sending data to the page to force this call with stuff like this
